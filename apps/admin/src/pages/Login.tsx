...
export default function Login() {
  ...
  return (
    <div className="min-h-screen flex bg-bg">
      <div className="hidden lg:flex w-1/2 flex-col justify-between bg-gradient-to-br from-leaf-800 via-leaf-700 to-leaf-900 p-12 text-white">
        <div className="flex items-center gap-3">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10 backdrop-blur">
            <BasketIcon className="text-harvest-300" size={26} />
          </div>
          <div>
              <div className="text-2xl font-black tracking-tight leading-none">Duhahe Market</div>
              <div className="text-xs text-leaf-200 mt-1">Duhahe · {t('fastDelivery')}</div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="mt-8 grid grid-cols-3 gap-3 max-w-md">
            {[
              ...
              { icon: LeafIcon, label: 'Farm fresh', sub: '128 products' },
            ].map((f, i) => (
        ...
        <div className="text-[11px] text-leaf-300">© {new Date().getFullYear()} Duhahe Market — admin console</div>
      </div>

      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-sm">
          <div className="lg:hidden flex items-center gap-3 mb-8">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br from-leaf-500 to-leaf-700">
              <BasketIcon className="text-white" size={24} />
            </div>
            <div className="text-2xl font-black tracking-tight">
              Duhahe <span className="text-leaf-600">Market</span>
            </div>
          </div>
          ...
          <div className="mt-4 flex justify-end gap-1">
            {(['en', 'kin', 'fr'] as const).map((option) => (
              <button key={option} onClick={() => setLang(option)} className={`rounded-lg border px-3 py-1.5 text-xs font-bold ${lang === option ? 'border-leaf-600 bg-leaf-50 text-leaf-700' : 'border-line text-muted'}`}>
                {option === 'kin' ? 'Kinyarwanda' : option === 'fr' ? 'Français' : 'English'}
              </button>
            ))}
          </div>

          <div className="mt-4 rounded-xl bg-leaf-50 border border-leaf-100 px-4 py-3 text-xs text-leaf-800 leading-relaxed">
